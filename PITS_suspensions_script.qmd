...
corrected end date anomalies in the suspensions
...
```{r}
# read in data
PITS_ListIncidents_2023 <- read_csv("PITS_Data/new_PITS_ListIncidents_2023.csv")
...
PITS_ListIncidents_2024 <- read_csv("PITS_Data/new_PITS_ListIncidents_2024.csv")

PITS_ListIncidents_2024 <- PITS_ListIncidents_2024[,-(19:23)] 

PITS_ListIncidents_2025 <- read_csv("PITS_Data/new_PITS_ListIncidents_Jan-Dec23_2025.csv")
PITS_ListIncidents_2025$Time <- as.character(PITS_ListIncidents_2025$Time)
PITS_ListIncidents_2025$`Incident ID` <- as.character(PITS_ListIncidents_2025$`Incident ID`)
...
PITS_susp <- read_csv("PITS_Data/PITS_ListSuspensions2023_Dec23_2025.csv")
# PITS_susp <- PITS_susp %>% 
#   select(-`Person Name`)
...
The `echo: false` option disables the printing of code (only output is displayed). \## Standardize Location format \# read in MyPC data

```{r}
MyPC_2023 <- read_csv("MyPC_data/MyPC_2023_siteAndLocationUsage.csv", 
                      skip = 15)
MyPC_2024 <- read_csv("MyPC_data/MyPC_2024_siteAndLocationUsage.csv", 
                      skip = 15)
MyPC_2025 <- read_csv("MyPC_data/MyPC_Dec23_2025_siteAndLocationUsage.csv", 
                      skip = 15)
...
```
write.csv(PITS_public1, "processed_data/PITS_public1.csv")
...
# correct some error dates
```{r}
PITS_public$`End Date`[PITS_public$`Incident ID` == "23896"] <- "2024-09-19"
PITS_public$`End Date`[PITS_public$`Incident ID` == "25180"] <- "2026-05-10"
...
# }
# 
# # Filter out rows where EndDate is before StartDate
# PITS_public <- PITS_public[PITS_public$`End Date` >= PITS_public$`Start Date`, ] # removed 4

PITS_public <- PITS_public %>%
  mutate(
    # Convert string "NA" to real NA across both suspension columns
    `Suspension Status` = na_if(`Suspension Status`, "NA"),
    `Suspension Type`   = na_if(`Suspension Type`,   "NA"),
    
    Suspension_length = case_when(
      is.na(`End Date`) | is.na(`Start Date`)  ~ NA_real_,
      `End Date` < `Start Date`                ~ NA_real_,
      TRUE ~ as.numeric(`End Date` - `Start Date`)
    ),
    
    # 1 only when there is an actual suspension type present
    susp_binary = ifelse(`Suspension Type` == "Suspended customer", 1, 0)
  )
```
...
# Check how many susp_binary NAs exist — should be 0 after your mutate fix
sum(is.na(PITS_public2$susp_binary))

# Always specify the join key explicitly
susp_but_no_followup <- anti_join(
  followup_public, 
  suspension_public, 
  by = "Incident ID"
)
...
write.csv(PITS_public2, "processed_data/PITS_public2.csv")

write.csv(MyPC_public, "processed_data/MyPC_public.csv")
```

# shareable format for the data
```{r}
PITS_public_share <- PITS_public2 %>% 
  select(-Location_Standard, -`Suspension Note`)
...
MyPC_public_share <- MyPC_public %>% 
  select(-Location, -Location_Standard, -Site, -Resource, -`Login ID`)
