...
server <- function(input, output, session) {
  ...
  # PITS filtered by date + location + day of week
  data <- reactive({
    df <- PITS_public %>%
    ...
    if (length(input$selected_days) > 0)
      df <- df %>% filter(PITS_weekday %in% input$selected_days)
    df
  })

  # MyPC filtered by date + location + day of week
  data2 <- reactive({
    df <- MyPC_public %>%
    ...
    if (!"All" %in% input$Location)
      df <- df %>% filter(Location_masked %in% input$Location)
    if (length(input$selected_days) > 0)
      df <- df %>% filter(MyPC_weekday %in% input$selected_days)
    df
  })
  ...
  # ratio_data filtered by date + location + day of week
  ratio_filtered <- reactive({
    ...
    if (!"All" %in% input$Location)
      df <- df %>% filter(Location_masked %in% input$Location)
    if (length(input$selected_days) > 0)
      df <- df %>% filter(weekdays(date) %in% input$selected_days)
    df
  })

  # susp_ratio_data filtered by date + location + day of week
