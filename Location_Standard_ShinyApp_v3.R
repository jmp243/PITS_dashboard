...
PITS_incident_counts <- PITS_public %>%
  group_by(Location_Standard, date = as.Date(Date2)) %>%
  summarise(incident_count = n_distinct(`Incident ID`), .groups = "drop")

MyPC_counts <- MyPC_public %>%
  group_by(Location_Standard, date = booking_date) %>%
  summarise(user_count = n(), .groups = "drop")

ratio_data <- MyPC_counts %>%
  full_join(PITS_incident_counts, by = c("Location_Standard", "date")) %>%
  mutate(
...
PITS_susp_deduped <- PITS_public %>%
  ...
  distinct(`Incident ID`, Location_Standard, Date2, Suspension_length)
...
susp_ratio_data <- PITS_incident_counts %>%
  left_join(PITS_susp_counts, by = c("Location_Standard", "date")) %>%
  mutate(
...
theme <- create_theme(
  bs4dash_color(lime = "#52A1A5", olive = "#4A9094", purple = "#8965CD"),
  bs4dash_status(primary = "#E1EDED", info = "#E4E4E4")
)

locations     <- sort(unique(PITS_public$Location_Standard))
...
server <- function(input, output, session) {
  ...
  data <- reactive({
    ...
    if (!"All" %in% input$Location)
      df <- df %>% filter(Location_Standard %in% input$Location)
    df
  })
  ...
  output$plot_MyPC <- renderPlotly({
    ...
    n_loc    <- length(unique(hu$Location_Standard))
    ...
    plot_ly(hu,
            ...
            y = ~count, color = ~Location_Standard,
            type = "bar", colors = pal,
  ...
  output$plot_PITS_by_type_location_time <- renderPlotly({
    ic <- data() %>%
      ...
      distinct(`Incident ID`, parsed_categories, Location_Standard, Date2, hour_label) %>%
  ...
  output$Violin_PITS_Susp_length <- renderPlotly({
    ...
    plot_ly(data = sd, x = ~Location_Standard, y = ~Suspension_length,
            type = "violin",
